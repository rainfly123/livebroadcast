...
def QueryAll(shop, keywords=None):
    ...
    if keywords != None:
        sql = "select live.liveid, title, snapshot, persons, rtmp_live_url,ownerid,public from live,\
           ...
           and live.title like '%{1}%' order by live.startime desc".format(shop, keywords)
    else:
        sql = "select live.liveid, title, snapshot, persons, rtmp_live_url,ownerid,public from live,\
           owner   where live.liveid=owner.liveid and live.state = 1 and live.shop={0} order by live.startime desc".format(shop)
    ...
    if keywords != None:
        ...
        sql = "select live.liveid, title, snapshot, supports, playback_hls_url,ownerid,public from live,\
           owner  where live.liveid=owner.liveid and live.state = 2 and live.shop={0} order by live.startime desc".format(shop)
...
def GoodsLive(goodid):
    ...
    con = getConn()
    cur =  con.cursor()
    sql = "select live.title, live.snapshot, live.state, live.rtmp_live_url,\
          live.playback_hls_url, date_format(live.startime, '%Y-%m-%d %H:%i'), live.liveid \
          from products,live where live.liveid = products.liveid and products.goodsid = {0}".format(goodid)
    cur.execute(sql)
    res = cur.fetchall()
    ...
    if len(res) >= 1 :
        ...
        result['snapshot'] = live[1]
        result['state'] = live[2]
        result['rtmp_live_url'] = live[3]
        result['playback_hls_url'] = live[4]
        result['startime'] = live[5]
        result['liveid'] = live[6]
        return result

    result['code'] =  5
    result['message'] =  "no live"
    return result
